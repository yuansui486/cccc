# OneColleague v0.4.11 Release Notes
...
Logging into Weixin only connects the account; it does not automatically tell OneColleague which Weixin bot chat should receive group messages. The IM Bridge settings panel now makes that distinction visible after login and explains the next step: send `/subscribe` in the target Weixin bot chat, then approve the request from Pending Requests.
