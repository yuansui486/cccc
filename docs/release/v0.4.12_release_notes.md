# OneColleague v0.4.12 Release Notes
...
Key improvements:
- `onecolleague update` command with install-source detection
...
This release also intentionally backs away from automatically generating Codex project `.codex/config.toml` files from OneColleague actor private environment settings. Codex provider customization remains explicit through Codex's own login/configuration and command-line override mechanisms.
