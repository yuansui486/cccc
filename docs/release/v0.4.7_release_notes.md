# OneColleague v0.4.7 Release Notes
...
Notably, the default `onecolleague` entry path now respects top-level `--host` / `--port` overrides throughout supervised Web startup and restart, and Kimi runtime defaults were updated to match the current preferred path.
