# OneColleague v0.4.15 Release Notes
...
The main theme is operational maturity. OneColleague now treats groups, capabilities, and browser-backed runtimes as durable systems with explicit state, safer copy boundaries, better UI feedback, and stronger failure semantics.
...
The old blueprint/template model copied a curated whitelist of fields. That approach became harder to maintain as OneColleague groups gained ledgers, memory, attachments, assistants, automation, browser state, capability state, and richer settings. Copy Groups flips the model: export durable group state by default, then exclude unsafe or runtime-only material.
...
## Slash Commands Route Through OneColleague

`v0.4.15` adds a stronger slash-command path for capability workflows.

Most importantly, `/install` is routed through the OneColleague capability registry instead of leaving the model to decide whether to install a local Codex skill package, a OneColleague skill, an MCP toolpack, or something else. The default install path now tells the agent to use `onecolleague_capability_install` with group scope, so installed skills appear in the group's slash command surface and remain manageable through Capability Center.

Chat also gained capability-backed slash command discovery and dispatch. This makes frequently used capability actions easier to invoke while preserving OneColleague's central lifecycle and policy checks.
...
Instead of relying primarily on repeated screenshot capture or CDP screencast frames, OneColleague can now expose OneColleague-owned Xvfb browser sessions through a localhost `x11vnc` server and a noVNC Web viewer. Browser automation and delivery still use the daemon-owned CDP/Playwright session; VNC is only the visual and interactive viewer transport.

This separation matters. The embedded viewer no longer has to compete with the same CDP path used for browser automation and ChatGPT message delivery. On Linux/container setups with a OneColleague-owned Xvfb display and `x11vnc` available, the viewer should feel closer to a remote desktop surface. When VNC is disabled, unsupported, or unsafe, OneColleague falls back to the built-in CDP screencast viewer.

For safety, OneColleague does not attach VNC to an inherited host desktop display. VNC is only enabled when the runtime knows the display is dedicated to the projected browser session. This avoids exposing unrelated local windows through the embedded browser panel.
...
This is intentionally conservative. Repeating an old failed batch into a later ChatGPT message can corrupt the conversation more severely than asking the user to inspect or retry explicitly. The Web UI now has better delivery-state projection so users can see when OneColleague is submitting, submitted, pending, ambiguous, or failed instead of guessing from a silent browser automation attempt.
...
Markdown code blocks now render through OneColleague's own code block UI without redundant nested `<pre><code>` containers, improving density and readability for long technical replies. The persistent composer and group store were tightened so selected group state and composer ownership stay aligned earlier in app startup. Settings initialization and Voice Secretary composer hooks were adjusted to remove stale hook warnings and keep lint clean.
...
- duplicate, migrate, or back up full OneColleague groups with Copy Groups instead of legacy blueprints,
- manage capability discovery, installation, visibility, source instances, and removal from a more complete Capability Center,
- use `/install` through the OneColleague capability lifecycle rather than an ambiguous model-chosen install path,
...
In short, `v0.4.15` is the release where OneColleague replaces fragile group templates with durable group copying, turns capability management into a real lifecycle, and makes browser-backed Web Model operation more observable, safer, and easier to recover.
