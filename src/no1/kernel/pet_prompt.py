    ...
    *,
    ...
    decision_contract = "\n".join(
        [
            ...
            "- When review_packet.task_evidence or pet-context.task_evidence is present, treat each item as a hypothesis packet built from code-side evidence. Re-rank it against live state; do not mirror it mechanically.",
            "- Finish every pet_review with exactly one onecolleague_pet_decisions call: action=replace with the full current decision list, or action=clear.",
            "- Default to draft_message. Valid surfaced actions are draft_message, task_proposal, and restart_actor.",
            "- Do not call onecolleague_message_send, onecolleague_message_reply, or visible file-send tools from Pet; surface drafts through pet decisions instead.",
            "- When a surfaced decision is non-trivial, include confidence=low|medium|high and a short reasoning_brief grounded in current evidence.",
            ...
            "- pet_profile_refresh is separate: if an unread notify has data.context.kind=pet_profile_refresh, update only onecolleague_agent_state(action=update, actor_id=pet-peer, user_model=...) from its sample_packet and do not touch onecolleague_pet_decisions.",
        ]
    )
    header_lines = [
