...
1. Recall before write: `onecolleague_memory(action="search", query=...)` then `onecolleague_memory(action="get", path=...)`.
...
2. Use `onecolleague_memory(action="search")` for candidate snippets, then `onecolleague_memory(action="get")` for exact lines.
3. Run recall before planning/high-impact edits to avoid contradiction with prior decisions.
4. On start/resume, consume `onecolleague_bootstrap.memory_recall_gate` first, then expand with manual search/get if needed.
""",
    "consolidation": """## Memory Consolidation Workflow

1. Check context pressure: `onecolleague_memory_admin(action="context_check", messages=[...])`.
2. If compaction is needed, run `onecolleague_memory_admin(action="compact", ...)`.
3. Flush durable conversation deltas: `onecolleague_memory_admin(action="daily_flush", messages=[...])`.
4. Promote stable long-horizon knowledge with `onecolleague_memory(action="write", target="memory", ...)`.
5. Re-sync index after large edits: `onecolleague_memory_admin(action="index_sync", mode="scan"|"rebuild")`.
""",
