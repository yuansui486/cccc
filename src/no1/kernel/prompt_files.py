...
PREAMBLE_FILENAME = "ONECOLLEAGUE_PREAMBLE.md"
HELP_FILENAME = "ONECOLLEAGUE_HELP.md"
...
def load_builtin_help_markdown() -> str:
    """Load the built-in OneColleague help markdown bundled in the package."""
    try:
        import importlib.resources

        files = importlib.resources.files("no1.resources")
        return (files / "onecolleague-help.md").read_text(encoding="utf-8")
    except Exception:
        try:
            p = Path(__file__).resolve().parents[1] / "resources" / "onecolleague-help.md"
