"""Built-in OneColleague install capability definition."""
...
INSTALL_CAPABILITY_RECORD = {
    "name": "install",
    "description_short": (
        "Install skills, MCP toolpacks, OneColleague capabilities, and GitHub repositories through the "
        "OneColleague capability registry, then enable them for the group slash surface."
    ),
    "use_when": (
        "The user types /install followed by a skill name, capability id, repo slug, URL, or local path.",
        "A requested capability needs to be imported into the OneColleague capability registry and enabled for the group slash surface.",
    ...
    "gotchas": (
        ...
        "Default /install behavior is OneColleague capability installation, not Codex local skill-package installation.",
    ),
    "evidence_kind": "installed/enabled target, source, destination or capability_id, and verification result",
    "capsule_text": (
        "You are the install skill for OneColleague agents.\n\n"
        "Default /install behavior is OneColleague capability installation, not Codex local skill-package installation.\n\n"
        "Default action: call onecolleague_capability_install for the target with scope=group. "
        "The install operation resolves the target into OneColleague capability records, imports them into the registry, "
        "enables group scope, and returns use-ready capability ids. "
        "Activate, assign, autoload, or use only after the OneColleague capability record exists.\n\n"
        ...
        "2. Prefer onecolleague_capability_install for /install targets. Fall back to capability search/import only "
        ...
        "4. For GitHub repositories, remote SKILL.md URLs, and local SKILL.md files/directories, install the minimal matching OneColleague capability records through onecolleague_capability_install. "
        "Do not bypass the registry by installing into Codex's local skills directory.\n"
        "5. For GitHub skill repositories, pass the repository URL or owner/repo as onecolleague_capability_install.target. If the repo "
        "contains multiple skills/*/SKILL.md files, the daemon must import and enable each SKILL.md as its own OneColleague "
        "skill capability; do not create one aggregate repository skill.\n"
        "6. Any additional activate, assign, autoload, or use step must operate against the imported OneColleague capability_id.\n"
        ...
        "Pitfalls:\n"
        "- Do not treat OneColleague /install as a full local Codex skill package install.\n"
        "- Do not install into Codex's local skills directory or use the Codex skill-installer workflow for /install.\n"
        "- Generic URLs and local paths must go through onecolleague_capability_install and become OneColleague capability records; report unsupported only when the install operation rejects the target.\n"
        ...
        "Verification:\n"
        "- The final response names exactly what OneColleague capability was installed, enabled for group scope, and how it was verified.\n"
        "- If blocked, the response names the blocker and owner instead of pretending installation succeeded."
    ),
    "tags": ("install", "skill", "mcp", "capability", "github", "repository", "onecolleague-glue"),
