...
class AutomationManager:
            ...
            for actor in roster:
                ...
                for ev in all_events:
                    ...
                    if is_reply_required and not is_replied:
                        ...
                        item_lines.append(
                            f"REPLY REQUIRED: event_id={ev_id} (since {ev_ts}). Reply via onecolleague_message_reply(event_id={ev_id}, ...)."
                        )
                        continue
                    ...
                    if is_attention and not is_acked:
                        ...
                        due_item_keys.append(item_key)
                        item_lines.append(
                            f"IMPORTANT awaiting ACK: event_id={ev_id} (since {ev_ts}). Use onecolleague_inbox_mark_read(event_id={ev_id})."
                        )
                    ...
                    if unread_dt is not None:
                        ...
                        if cfg.nudge_max_repeats_per_obligation <= 0 or repeat < int(cfg.nudge_max_repeats_per_obligation):
                            ...
                            if (now - base_dt).total_seconds() >= float(due_after):
                                due_item_keys.append(item_key)
                                item_lines.append(
                                    f"Unread backlog: oldest from {oldest_unread_ts}. Use onecolleague_inbox_list() to review."
                                )
    ...
    def _check_help_nudge(self, group: Group, cfg: AutomationConfig, now: datetime) -> None:
        """Remind running actors to refresh the help playbook via onecolleague_help.
        ...
        for aid, runner_kind, nudge_kind in to_notify:
            if nudge_kind == "mind_context":
                message = (
                    "Run `onecolleague_help` now, then refresh `onecolleague_agent_state` "
    ...
    def on_new_message(self, group: Group) -> None:
        ...
        threading.Thread(target=_run_memory_auto, name=f"onecolleague-memory-auto-{group_id or 'group'}", daemon=True).start()
