...
def handle_web_model_runtime_wait_next_turn(args: Dict[str, Any]) -> DaemonResponse:
    ...
    if _clean_text(actor.get("runtime")).lower() != "web_model":
        return _error(
            ...
            "onecolleague_runtime_wait_next_turn is only available for runtime=web_model actors",
    ...
    if not messages:
        ...
        return DaemonResponse(
            ...
            result={
                ...
                "instructions": "No unread work is available. Call onecolleague_runtime_wait_next_turn again after a short wait.",
            },
        )
    ...
    turn = {
        ...
        "instructions": (
            "Process this coalesced OneColleague turn. Use OneColleague MCP tools for visible replies, handoffs, repo edits, "
            "shell/git work, validation, and evidence. When finished, call onecolleague_runtime_complete_turn; "
...
def handle_web_model_runtime_complete_turn(args: Dict[str, Any]) -> DaemonResponse:
    ...
    if _clean_text(actor.get("runtime")).lower() != "web_model":
        return _error(
            "invalid_actor_runtime",
            "onecolleague_runtime_complete_turn is only available for runtime=web_model actors",
            details={"group_id": group_id, "actor_id": actor_id},
        )
    if not _web_model_actor_running(group_id, actor_id, actor):
