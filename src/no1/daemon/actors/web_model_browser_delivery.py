...
_LOG = logging.getLogger("no1.daemon.web_model.browser_delivery")
...
def _build_web_model_bootstrap_seed(group: Any, actor: Dict[str, Any]) -> str:
    ...
    transport_note = (
        "[OneColleague] Web transport:\n"
        "- This ChatGPT conversation is the browser surface for the actor above.\n"
        "- Browser-injected messages are already delivered in chat; do not call onecolleague_runtime_wait_next_turn for them.\n"
        "- Use OneColleague MCP tools for visible replies, handoffs, local workspace work, validation, and evidence.\n"
        "- For non-trivial local development work, default to onecolleague_code_exec so repo reads, patches, tests, diffs, "
        ...
        "in this chat; tell the user to switch to a GPT-5.x ChatGPT session that can see the OneColleague connector.\n"
        "- Text typed only in this web chat is not delivered to OneColleague users or peers."
    )
    return "\n\n".join(
        [
...
def schedule_web_model_browser_delivery(
    *,
    ...
    threading.Thread(
        target=_worker,
        name=f"onecolleague-web-model-browser-delivery-{gid}-{aid}",
        daemon=True,
    ).start()
    return True
