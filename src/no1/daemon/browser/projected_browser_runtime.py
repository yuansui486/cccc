...
class _ProjectedVncServer:
    ...
    def start(cls, *, display: str, display_owned: bool = False) -> tuple[Optional["_ProjectedVncServer"], str]:
        ...
        if not bool(display_owned):
            return None, "display_not_onecolleague_owned"
...
def launch_projected_browser_runtime(
    *,
    ...
    display_owner = "onecolleague_xvfb" if display_owned else ""
        ...
        if channel:
            ...
            if bool(require_system_browser_cdp):
                raise RuntimeError(
                    ...
                    "install Google Chrome or Microsoft Edge and close any existing OneColleague ChatGPT browser using the same profile"
                )
...
class ProjectedBrowserSession:
    def __init__(
        ...
        *,
        ...
        self._thread = threading.Thread(
            target=self._run,
            daemon=True,
            name=f"onecolleague-browser-{self.session_key[:48]}",
        )
    ...
    def attach_socket(self, sock: socket.socket, *, viewer_mode: str = "auto") -> bool:
        ...
        threading.Thread(
            ...
            daemon=True,
            name=f"onecolleague-browser-stream-{self.session_key[:48]}",
        ).start()
        return True
...
def _bridge_raw_sockets(left: socket.socket, right: socket.socket) -> None:
    ...
    try:
        ...
        pass
    threading.Thread(target=_pipe, args=(left, right), daemon=True, name="onecolleague-browser-vnc-left").start()
    threading.Thread(target=_pipe, args=(right, left), daemon=True, name="onecolleague-browser-vnc-right").start()
