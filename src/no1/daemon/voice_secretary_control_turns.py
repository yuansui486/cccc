...
def prepare_control_turn(
    *,
    ...
    lines = [
        base_text.strip(),
        "",
        "[OneColleague] SYSTEM PREFETCH: read_new_input already ran before this turn.",
...
def repair_control_text(*, text: str, diagnostics: Dict[str, Any]) -> str:
    ...
    if legacy_input_missing:
        lines.extend(
            [
                "[OneColleague] REPAIR HINT: previous legacy pointer turn did not fetch the secretary input.",
                "Call MCP tool onecolleague_voice_secretary_document(action=\"read_new_input\") before doing other work, then complete every missing action below:",
            ]
        )
    else:
        lines.extend(
            [
                "[OneColleague] REPAIR HINT: previous Voice Secretary turn did not complete the required output.",
                "Use the daemon-delivered input_envelope already included in this control turn; use MCP tool onecolleague_voice_secretary_document(action=\"read_new_input\") only for a legacy pointer with no envelope.",
