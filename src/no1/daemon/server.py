...
def _inject_actor_context_env(env: Dict[str, Any], *, group_id: str, actor_id: str) -> Dict[str, Any]:
    ...
    out["ONECOLLEAGUE_HOME"] = str(ensure_home())
    out["CCCC_HOME"] = out["ONECOLLEAGUE_HOME"]
...
class DaemonPaths:
    ...
    def sock_path(self) -> Path:
        return self.daemon_dir / "onecolleagued.sock"

    @property
    def addr_path(self) -> Path:
        # Cross-platform daemon endpoint descriptor (TCP fallback on Windows).
        return self.daemon_dir / "onecolleagued.addr.json"

    @property
    def pid_path(self) -> Path:
        return self.daemon_dir / "onecolleagued.pid"

    @property
    def log_path(self) -> Path:
        return self.daemon_dir / "onecolleagued.log"
...
def _remove_stale_socket(sock_path: Path) -> None:
    # Deprecated: daemon IPC is now cross-platform and uses `onecolleagued.addr.json`.
...
def serve_forever(paths: Optional[DaemonPaths] = None) -> int:
    ...
    lock_path = p.daemon_dir / "onecolleagued.lock"
    try:
