...
def stop_codex_app_server_for_pty_actor_if_needed(*, group_id: str, actor_id: str, pid: int = 0) -> bool:
    ...
    if not isinstance(actor, dict) or not actor_uses_codex_app_server_state(actor):
        return False
    if codex_app_supervisor.handle_remote_tui_exit(group_id=group_id, actor_id=actor_id, pid=pid):
        return True
