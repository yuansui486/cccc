...
_LOG = logging.getLogger("no1.daemon.serve_ops")
...
def start_automation_thread(
    *,
    ...
    t = threading.Thread(target=_automation_loop, name="onecolleague-automation", daemon=True)
    t.start()
    return t
...
def start_request_execution_thread(
    *,
    request_queue: Any,
    name: str = "onecolleague-request-worker",
) -> threading.Thread:
    t = threading.Thread(target=request_queue.run_forever, name=str(name or "onecolleague-request-worker"), daemon=True)
    t.start()
    return t
...
def start_space_jobs_thread(
    *,
    ...
    t = threading.Thread(target=_space_jobs_loop, name="onecolleague-space-jobs", daemon=True)
    t.start()
    return t
...
def start_space_sync_thread(
    *,
    ...
    t = threading.Thread(target=_space_sync_loop, name="onecolleague-space-sync", daemon=True)
    t.start()
    return t
...
def start_supervisor_watchdog_thread(
    *,
    ...
    t = threading.Thread(target=_supervisor_watchdog_loop, name="onecolleague-supervisor-watchdog", daemon=True)
    t.start()
    return t
...
def start_actor_activity_thread(
    *,
    ...
    t = threading.Thread(target=_actor_activity_loop, name="onecolleague-actor-activity", daemon=True)
    t.start()
    return t
...
def start_bootstrap_thread(
    *,
    ...
    t = threading.Thread(target=_bootstrap_after_listen, name="onecolleague-bootstrap", daemon=True)
    t.start()
    return t
