...
def _remote_search_skillsmp_records(*, query: str, limit: int) -> List[Dict[str, Any]]:
    ...
    if api_key:
        try:
            ...
            api_data = _pkg()._http_get_json_obj(
                api_url,
                headers={"Authorization": f"Bearer {api_key}", "User-Agent": "onecolleague-capability-sync/1.0"},
                timeout=timeout_s,
            )
    ...
    text = _pkg()._http_get_text(url, headers={"User-Agent": "onecolleague-capability-sync/1.0"}, timeout=timeout_s)
...
def _sync_anthropic_skills_source(catalog: Dict[str, Any], *, force: bool = False) -> int:
    ...
    try:
        ...
        # parent package namespace (no1.daemon.ops.capability_ops).
