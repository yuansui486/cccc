...
logger = logging.getLogger("no1.delivery")
...
def _render_system_notify_message_for_delivery(*, notify: SystemNotifyData, group: Optional[Group] = None) -> str:
    ...
    if context_kind == "voice_secretary_input":
        ...
        blocks = [
            ...
            "Legacy pointer notification: call MCP tool onecolleague_voice_secretary_document(action=\"read_new_input\") before doing other work.",
...
def render_headless_control_text(*, control_kind: str, body: str) -> str:
    ...
    if kind == "bootstrap":
        intro = (
            "[OneColleague] INTERNAL CONTROL: session bootstrap\n"
        ...
        intro = (
            "[OneColleague] INTERNAL CONTROL: system notification\n"
...
def render_batched_messages(messages: List[PendingMessage], *, reminder_after_index: Optional[int] = None) -> str:
    ...
    blocks: List[str] = []
    if len(messages) > 1:
        blocks.append(f"[onecolleague] {len(messages)} new messages:")
...
def request_flush_pending_messages(group: Group, *, actor_id: str) -> bool:
    ...
    threading.Thread(target=_run, name=f"onecolleague-delivery-flush-{gid}-{aid}", daemon=True).start()
    return True
