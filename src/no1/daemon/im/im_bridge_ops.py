    ...
    if proc.exists():
        for proc_dir in proc.iterdir():
            ...
            try:
                ...
                continue
            if "no1.ports.im.bridge" not in cmdline or gid not in cmdline:
                continue
            proc_home = _proc_onecolleague_home(pid)
    ...
    if proc.exists():
        for proc_dir in proc.iterdir():
            ...
            try:
                ...
                continue
            if "no1.ports.im.bridge" not in cmdline:
                continue
            proc_home = _proc_onecolleague_home(pid)
            if proc_home is None:
                continue
    ...
    *,
    ...
    if proc.exists():
        for proc_dir in proc.iterdir():
            ...
            try:
                index = argv.index("no1.ports.im.bridge")
            except ValueError:
                continue
            if index + 1 >= len(argv):
                continue
