"""ChatGPT page automation helpers for OneColleague Web Model actors.
...
def _chatgpt_tool_confirm_script() -> str:
    ...
    (args) => {
        ...
        for (const button of document.querySelectorAll("button")) {
            ...
            const clickedAt = Number(button.getAttribute("data-onecolleague-auto-confirm-clicked-at") || 0);
            ...
            const candidateId = `onecolleague-tool-confirm-${now}-${out.length}`;
            button.setAttribute("data-onecolleague-auto-confirm-candidate-id", candidateId);
...
def chatgpt_browser_profile_root(group_id: str = "", actor_id: str = "") -> Path:
    ...
    the ChatGPT browser surface rather than to a replaceable OneColleague actor id.
    """
    _ = (group_id, actor_id)
        ...
        try:
            candidate = page.evaluate(
                """selectors => {
                    ...
                    if (!best) return "";
                    const marker = "onecolleague-chatgpt-composer-input";
                    for (const node of document.querySelectorAll("[data-onecolleague-chatgpt-input-candidate]")) {
                        node.removeAttribute("data-onecolleague-chatgpt-input-candidate");
                    }
                    best.setAttribute("data-onecolleague-chatgpt-input-candidate", marker);
                    return `[data-onecolleague-chatgpt-input-candidate="${marker}"]`;
                }""",
                INPUT_SELECTORS,
            )
            ...
            last_error = str(exc)
        time.sleep(0.2)
    raise RuntimeError(last_error or "ChatGPT composer input not found; log in and enable the OneColleague connector")
    ...
    try:
        texts = page.evaluate(
            """selector => {
                ...
                [
                    ...document.querySelectorAll("[data-onecolleague-chatgpt-input-candidate]"),
        ...
        try:
            candidate_selector = page.evaluate(
                """() => {
                    ...
                    const marker = "onecolleague-chatgpt-send-candidate";
                    for (const node of document.querySelectorAll("[data-onecolleague-chatgpt-send-candidate]")) {
                        node.removeAttribute("data-onecolleague-chatgpt-send-candidate");
                    }
                    best.setAttribute("data-onecolleague-chatgpt-send-candidate", marker);
                    return `[data-onecolleague-chatgpt-send-candidate="${marker}"]`;
                }"""
            )
...
def _auto_confirm_page_tool_prompts(page: Any, *, max_clicks: int = TOOL_CONFIRM_MAX_CLICKS) -> dict[str, Any]:
    ...
    for raw in candidates[: max(1, min(int(max_clicks or 1), TOOL_CONFIRM_MAX_CLICKS))]:
        ...
        if not candidate_id:
            continue
        selector = f'button[data-onecolleague-auto-confirm-candidate-id="{candidate_id}"]'
        try:
            locator = page.locator(selector).first
            if locator.count() <= 0:
                continue
...
def _submission_echo_needles(prompt: str) -> list[str]:
    ...
    if not normalized:
        return []
    needles: list[str] = []
    batch_match = re.search(r"\[onecolleague\]\s+Browser batch\s+([^\s]+)", normalized)
    if batch_match:
...
def build_chatgpt_web_model_health_snapshot(
    *,
    ...
    if pending_bind_delivery:
        ...
        delivery_reason = "OneColleague is currently injecting this batch into the ChatGPT browser session."
    ...
    return {
        "schema": "no1.web_model.health.v1",
...
def _page_pending_delivery_id(page: Any) -> str:
    try:
        value = page.evaluate("() => sessionStorage.getItem('onecolleague_pending_new_chat_delivery_id') || ''")
    except Exception:
        return ""
...
def _mark_page_pending_delivery(page: Any, delivery_id: str) -> None:
    ...
    if not token:
        return
    try:
        page.evaluate("value => sessionStorage.setItem('onecolleague_pending_new_chat_delivery_id', value)", token)
    except Exception:
        pass
