...
def _truncate_text(text: str, *, max_bytes: int) -> tuple[str, bool]:
    ...
    return clipped.rstrip() + "\n[onecolleague] output truncated", True
...
def _reader_thread(stream: Any, output: "queue.Queue[str]") -> None:
    try:
        ...
        output.put(f"\n[onecolleague] exec output reader failed: {exc}")
...
def _assert_expected_sha256(path: Path, expected_sha256: str) -> str:
    ...
    if current.lower() != expected:
        raise _mcp_error(
            code="stale_file",
            message=(
                "file changed since it was read; re-read onecolleague_repo(action=read), then retry with the new sha256 "
                f"(current_sha256={current})"
            ),
            recommended_action="Re-read the target file with onecolleague_repo(action='read') and retry the write/replace with the returned sha256.",
        )
    return current
...
def _find_line_sequence(lines: List[str], needle: List[str]) -> int:
    ...
    if not matches:
        raise _mcp_error(
            ...
            recommended_action="Re-read the current file around the intended location, then retry with a smaller exact Codex patch or use onecolleague_repo_edit(action='replace').",
        )
    if len(matches) > 1:
        raise _mcp_error(
            code="patch_context_ambiguous",
            message="Codex patch context matched multiple locations",
            recommended_action="Add more surrounding context to the Codex patch or use onecolleague_repo_edit(action='replace', expected_replacements=1).",
        )
    return matches[0]
...
def repo_tool(
    *,
    ...
    if act == "replace":
        ...
        if truncated:
            raise MCPError(code="file_too_large", message="file is too large for replace; use a smaller file or onecolleague_shell")
        count = text.count(old)
        if count <= 0:
            raise _mcp_error(
                code="old_text_not_found",
                message="old_text was not found; re-read the file and retry with exact current text",
                recommended_action="Call onecolleague_repo(action='read') for the target path, copy the exact current text, and retry replace with expected_sha256.",
            )
...
def git_tool(
    *,
    ...
    if act == "status":
        ...
        raise MCPError(code="invalid_action", message="onecolleague_git action must be status|diff|log|add|commit")
