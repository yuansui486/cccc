...
def _handle_memory_namespace(
    ...
    *,
    ...
    # Keep build_memory_guide injectable for bootstrap/help payloads, but onecolleague_memory
    # no longer exposes guide action in hard-cut ReMe mode.
    _ = build_memory_guide
    if name == "onecolleague_memory":
        ...
        raise mcp_error_cls(
            "invalid_request",
            "onecolleague_memory action must be one of: layout_get/search/get/write",
        )

    if name == "onecolleague_memory_admin":
        gid = resolve_group_id(arguments)
        ...
        raise mcp_error_cls(
            "invalid_request",
            "onecolleague_memory_admin action must be one of: index_sync/context_check/compact/daily_flush",
        )
