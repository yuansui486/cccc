...
def capability_use(
    *,
    ...
    if not call_tool:
        ...
        return out
    if call_tool == "onecolleague_capability_use":
        raise MCPError(
            code="capability_use_invalid_tool",
            message="onecolleague_capability_use cannot recursively call itself",
            details={},
        )
    ...
    if call_tool == "onecolleague_memory":
        ...
        if mem_admin_action in {"index_sync", "context_check", "compact", "daily_flush"}:
            skip_actor_injection = True
    elif call_tool == "onecolleague_space":
