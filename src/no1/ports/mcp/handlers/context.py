...
def _resolve_task_type(arguments: Dict[str, Any]) -> Optional[str]:
    ...
    if raw and task_type is None:
        raise MCPError(
            ...
            message="onecolleague_task type must be one of: free|standard|optimization",
        )
    return task_type
        ...
        _call_daemon_or_raise(
            {
                ...
                "args": {
                    ...
                    "message": (
                        "Updated: your actor note. Run `onecolleague_help` now to refresh your effective playbook; "
...
def _handle_context_namespace(
    ...
    *,
    ...
    if name == "onecolleague_context_get":
    ...
    if name == "onecolleague_context_sync":
        gid = resolve_group_id(arguments)
    ...
    if name == "onecolleague_coordination":
        gid = resolve_group_id(arguments)
        by = resolve_self_actor_id(arguments)
        ...
        raise MCPError(code="invalid_request", message="onecolleague_coordination action must be get|update_brief|add_decision|add_handoff")

    if name == "onecolleague_task":
        gid = resolve_group_id(arguments)
        by = resolve_self_actor_id(arguments)
        ...
        raise MCPError(code="invalid_request", message="onecolleague_task action must be list|create|update|move|restore|delete")

    if name == "onecolleague_agent_state":
        gid = resolve_group_id(arguments)
        ...
        if action != "update":
            raise MCPError(code="invalid_request", message="onecolleague_agent_state action must be get|update|clear")
