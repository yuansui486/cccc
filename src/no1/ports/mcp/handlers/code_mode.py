...
CODE_MODE_EXEC_TOOL = "onecolleague_code_exec"
CODE_MODE_WAIT_TOOL = "onecolleague_code_wait"
...
_COMMON_WORK_LOOPS = [
    {
        ...
        "steps": [
            "onecolleague_git(action='status')",
            "onecolleague_git(action='diff')",
            "onecolleague_repo(action='read', start_line/end_line as needed)",
            "onecolleague_shell or onecolleague_exec_command for focused tests",
            "onecolleague_message_reply with findings/evidence",
            "onecolleague_runtime_complete_turn for the processed event ids",
        ],
    },
    {
        "name": "patch_safely",
        "steps": [
            "onecolleague_repo(action='read') to get content and sha256",
            "onecolleague_repo_edit(action='replace' or 'multi_replace', expected_sha256=sha256) for exact edits",
            "onecolleague_apply_patch only for Codex *** Begin Patch file patches",
            "onecolleague_git(action='diff')",
    ...
    },
    {
        "name": "attachments",
        "steps": [
            "onecolleague_file(action='read', rel_path=...) for text blobs delivered by CCCC",
            "onecolleague_file(action='blob_path', rel_path=...) for binary/local inspection",
            "create output files under the active scope",
            "onecolleague_file(action='send', path=..., text=...) to return files as OneColleague attachments",
        ],
    },
    {
        "name": "finish_turn",
        "steps": [
            "send visible result with onecolleague_message_reply or onecolleague_message_send",
            "refresh onecolleague_agent_state when focus/next_action/blockers changed",
            "call onecolleague_runtime_complete_turn with the turn/event ids from the delivered work",
        ],
    },
]
_TOOL_HELP_ALIASES = {
...
_TOOL_HELP_COMPACT_NOTES = {
    "repo": [
        "Use onecolleague_repo(action='read') to inspect text and get sha256.",
        "Use onecolleague_repo_edit(action='replace'|'multi_replace', expected_sha256=...) for exact small edits.",
        "Use onecolleague_apply_patch only for Codex *** Begin Patch file patches.",
        "After edits, inspect onecolleague_git(action='diff') and run focused validation.",
    ],
    "file": [
        "OneColleague attachments are blob references, not browser uploads.",
        "Use onecolleague_file(action='read', rel_path=...) for UTF-8 text attachments.",
        "Use onecolleague_file(action='blob_path', rel_path=...) for binary/local inspection.",
        "Use onecolleague_file(action='send', path=..., text=...) to return active-scope files as chat attachments.",
    ],
    "shell": [
        "Use onecolleague_shell for short one-shot commands.",
        "Use onecolleague_exec_command plus onecolleague_write_stdin for long-running or interactive commands.",
        "Keep cwd under the active scope and cap output with max_output_bytes.",
    ],
    "message": [
        "Visible OneColleague delivery must use onecolleague_message_send or onecolleague_message_reply.",
        "Use onecolleague_message_reply with event_id/reply_to for an existing thread.",
        "Use onecolleague_tracked_send only for durable delegation that needs a task plus linked visible message.",
        "Verify to/reply_to before sending; avoid routine @all.",
    ],
    "state": [
        "Use onecolleague_agent_state to keep focus, next_action, what_changed, active_task_id, and blockers fresh.",
        "Update state at real transitions; do not let status replies replace actual work.",
    ],
    "runtime_complete": [
        "Use onecolleague_runtime_complete_turn after processing Web Model turn event ids.",
    ...
    "code_exec": [
        "Use onecolleague_code_exec for multi-step read/patch/test/diff/report loops.",
...
_CODE_MODE_NESTED: ContextVar[bool] = ContextVar("onecolleague_code_mode_nested", default=False)
...
def _reject_unsupported_source(source: str) -> None:
    if len(source) > _MAX_SOURCE_CHARS:
        raise MCPError(
            ...
            details={"recommended_action": "Split the work into a smaller onecolleague_code_exec cell or move large data into files under the active scope."},
        )
    if re.search(r"(^|[^\w$])require\s*\(", source):
        raise MCPError(
            code="unsupported_js",
            message="onecolleague_code_exec does not expose require()",
            details={"recommended_action": "Use nested MCP tools such as tools.onecolleague_repo, tools.onecolleague_shell, or tools.onecolleague_exec_command instead of Node host APIs."},
        )
    if re.search(r"(^|[^\w$])import\s*\(", source) or re.search(r"(^|[^\w$])import\s+(['\"{*$A-Za-z_])", source):
        raise MCPError(
            code="unsupported_js",
            message="onecolleague_code_exec does not support import",
            details={"recommended_action": "Use nested MCP tools for filesystem, shell, git, and OneColleague operations; do not import Node modules in code mode."},
        )
...
def _find_node() -> str:
    ...
    raise MCPError(
        code="node_not_found",
        message="onecolleague_code_exec requires Node.js on the OneColleague server host; direct MCP tools remain available",
        details={"recommended_action": "Use direct onecolleague_repo/onecolleague_shell/onecolleague_git tools, or install Node.js on the OneColleague server host."},
    )
...
def _missing_cell_response(cell_id: str) -> Dict[str, Any]:
    return {
        ...
        "recommended_action": "Check the cell_id from the latest onecolleague_code_exec result; if the cell expired or belonged to another actor, start a new onecolleague_code_exec cell.",
    }
...
def _format_response(
    *,
    ...
    recommended_action = ""
    if status == "failed":
        recommended_action = "Read error_text, fix the JS or nested tool call, then rerun a smaller onecolleague_code_exec cell."
    elif running:
        recommended_action = "Call onecolleague_code_wait with this cell_id to collect more output or final status."
    elif truncated:
        recommended_action = (
    ...
    *,
    ...
    if code_mode_nested_call_active():
        raise MCPError(
            ...
            message="onecolleague_code_exec cannot be called from inside code mode",
...
def code_wait_tool(
    ...
    *,
    ...
    if code_mode_nested_call_active():
        raise MCPError(
            code="recursive_code_mode",
            message="onecolleague_code_wait cannot be called from inside code mode",
            details={"recommended_action": "Use yield_control() inside code mode; call onecolleague_code_wait only from the outer MCP client."},
        )
    ...
    if not cell_id:
        raise MCPError(
            ...
            details={"recommended_action": "Use the cell_id returned by onecolleague_code_exec when status=running."},
        )
...
const EXIT_SENTINEL = "__onecolleague_code_mode_exit__";
let pending = new Map();
let storedValues = {};
let nextToolId = 1;
let started = false;
...
function buildContext(toolsMetadata, initialStoredValues, workLoops, helpAliases, helpCompactNotes, helpCuratedTools, helpCuratedLoops) {
  ...
  Object.defineProperties(sandbox, {
    __onecolleague_bridge__: { value: buildBridge(), configurable: true },
    __onecolleague_tools_metadata_json__: { value: JSON.stringify(Array.isArray(toolsMetadata) ? toolsMetadata : []), configurable: true },
    __onecolleague_work_loops_json__: { value: JSON.stringify(Array.isArray(workLoops) ? workLoops : []), configurable: true },
    __onecolleague_help_aliases_json__: { value: JSON.stringify(helpAliases && typeof helpAliases === "object" ? helpAliases : {}), configurable: true },
    __onecolleague_help_compact_notes_json__: { value: JSON.stringify(helpCompactNotes && typeof helpCompactNotes === "object" ? helpCompactNotes : {}), configurable: true },
    __onecolleague_help_curated_tools_json__: { value: JSON.stringify(helpCuratedTools && typeof helpCuratedTools === "object" ? helpCuratedTools : {}), configurable: true },
    __onecolleague_help_curated_loops_json__: { value: JSON.stringify(helpCuratedLoops && typeof helpCuratedLoops === "object" ? helpCuratedLoops : {}), configurable: true },
    __onecolleague_stored_values_json__: { value: jsonString(initialStoredValues && typeof initialStoredValues === "object" ? initialStoredValues : {}), configurable: true },
    __onecolleague_exit_sentinel__: { value: EXIT_SENTINEL, configurable: true },
  ...
  const context = vm.createContext(sandbox, {
    name: "onecolleague_code_mode",
    codeGeneration: { strings: false, wasm: false },
  });
  const bootstrap = `
(() => {
  const bridge = globalThis.__onecolleague_bridge__;
  const toolsMetadata = JSON.parse(globalThis.__onecolleague_tools_metadata_json__ || "[]");
  const commonWorkLoops = Object.freeze(JSON.parse(globalThis.__onecolleague_work_loops_json__ || "[]"));
  const helpAliases = JSON.parse(globalThis.__onecolleague_help_aliases_json__ || "{}");
  const helpCompactNotes = JSON.parse(globalThis.__onecolleague_help_compact_notes_json__ || "{}");
  const helpCuratedTools = JSON.parse(globalThis.__onecolleague_help_curated_tools_json__ || "{}");
  const helpCuratedLoops = JSON.parse(globalThis.__onecolleague_help_curated_loops_json__ || "{}");
  const exitSentinel = String(globalThis.__onecolleague_exit_sentinel__ || "");
  let storedValues = JSON.parse(globalThis.__onecolleague_stored_values_json__ || "{}");
  delete globalThis.__onecolleague_bridge__;
  delete globalThis.__onecolleague_tools_metadata_json__;
  delete globalThis.__onecolleague_work_loops_json__;
  delete globalThis.__onecolleague_help_aliases_json__;
  delete globalThis.__onecolleague_help_compact_notes_json__;
  delete globalThis.__onecolleague_help_curated_tools_json__;
  delete globalThis.__onecolleague_help_curated_loops_json__;
  delete globalThis.__onecolleague_stored_values_json__;
  delete globalThis.__onecolleague_exit_sentinel__;
  ...
  define("tool_help", function tool_help(query = "", options = {}) {
    ...
    return {
      ...
      usage: "Call nested tools as await tools.<name>({...}). Prefer onecolleague_code_exec for multi-step read/patch/test/diff/report loops; use direct tools for one-step work.",
    };
  });
  ...
  define("setTimeout", function onecolleagueSetTimeout(callback, ms, ...args) {
    if (typeof callback !== "function") {
      throw new TypeError("setTimeout callback must be a function");
    }
    return bridge.setTimeout(callback, Number(ms) || 0, ...args);
  });
  define("clearTimeout", function onecolleagueClearTimeout(id) {
    return bridge.clearTimeout(id);
  });
  define("__onecolleague_export_stored_values__", function __onecolleague_export_stored_values__() {
    return JSON.stringify(storedValues);
  });
})();
`;
  new vm.Script(bootstrap, { filename: "onecolleague_code_exec_bootstrap.mjs" }).runInContext(context, { timeout: 2000 });
  return context;
}
...
function exportStoredValues(context) {
  ...
  try {
    const raw = vm.runInContext("__onecolleague_export_stored_values__()", context, { timeout: 1000 });
...
async function startCell(command) {
  ...
  try {
    ...
    const script = new vm.Script(`(async () => {\n${source}\n})()`, {
      filename: "onecolleague_code_exec.mjs",
    });
