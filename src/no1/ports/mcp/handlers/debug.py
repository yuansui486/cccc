...
def debug_tail_logs(*, group_id: str, actor_id: str, component: str, lines: int = 200) -> Dict[str, Any]:
    """Tail OneColleague local logs (developer mode only; user+foreman only)."""
    return _call_daemon_or_raise(
        {
            "op": "debug_tail_logs",
            "args": {
    ...
    *,
    ...
    if name == "onecolleague_debug":
        ...
        raise MCPError(
            ...
            message="onecolleague_debug action must be 'snapshot' or 'tail_logs'",
        )
    return None
