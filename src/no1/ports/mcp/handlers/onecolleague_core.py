...
from . import onecolleague_group_actor as _group_actor_mod
from . import context as _context_mod


_ONECOLLEAGUE_HELP_BUILTIN = _load_builtin_help_markdown().strip()
...
def _build_memory_recall_gate(*, group_id: str, actor_id: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    gate: Dict[str, Any] = {
        ...
        "note": (
            ...
            "If it is empty, expand with local onecolleague_memory(search/get)."
        ),
    }
    try:
    ...
    if isinstance(space_state, dict):
        ...
        if work_bound or memory_bound:
            ...
            if "pack:space" not in enabled_pack_ids:
                lines_space.append(
                    '- If `onecolleague_space` is hidden in this session, use `onecolleague_capability_use(tool_name="onecolleague_space", tool_arguments={"action":"status"})` to expose it.'
                )
            if work_bound:
                lines_space.append(
...
def inbox_mark_read(*, group_id: str, actor_id: str, event_id: str) -> Dict[str, Any]:
    ...
    if not eid:
        raise MCPError(
            ...
            details={"recommended_action": "Use onecolleague_inbox_list to get the unread event id, then mark that exact id read."},
        )
    return _call_daemon_or_raise(
...
def bootstrap(
    *,
    ...
    return {
        ...
        "next_calls": {
            "help": "onecolleague_help()",
            "project_info": "onecolleague_project_info()",
            "context_get": "onecolleague_context_get()",
            "inbox_list": 'onecolleague_inbox_list(kind_filter="all")',
            "memory_search": 'onecolleague_memory(action="search", query=...)',
            "interrupt_triage": (
