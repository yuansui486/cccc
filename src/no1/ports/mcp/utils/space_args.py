...
def _normalize_space_query_options_mcp(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if unknown_top:
        if any(k in {"language", "lang"} for k in unknown_top):
            raise MCPError(
                ...
                message=(
                    "onecolleague_space(action=query) does not support top-level language/lang. "
        ...
        raise MCPError(
            code="invalid_request",
            message=(
                "onecolleague_space(action=query) unsupported top-level args: "
    ...
    if options_raw is None:
        ...
        raise MCPError(code="invalid_request", message="onecolleague_space(action=query) options must be an object")
    ...
    if unsupported_options:
        if any(str(k or "").strip() in {"language", "lang"} for k in unsupported_options):
            raise MCPError(
                code="invalid_request",
                message=(
                    "onecolleague_space(action=query) options do not support language/lang. "
                    "NotebookLM query API has no language parameter; put language requirements in query text."
                ),
            )
        raise MCPError(
            code="invalid_request",
            message=(
                "onecolleague_space(action=query) unsupported options: "
    ...
    if "source_ids" in options:
        ...
        if raw_source_ids is None:
            ...
            raise MCPError(
                code="invalid_request",
                message="onecolleague_space(action=query) options.source_ids must be an array of non-empty strings",
            )
        else:
            source_ids: List[str] = []
            for idx, item in enumerate(raw_source_ids):
                ...
                if not sid:
                    raise MCPError(
                        code="invalid_request",
                        message=f"onecolleague_space(action=query) options.source_ids[{idx}] must be a non-empty string",
                    )
