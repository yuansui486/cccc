...
_ONECOLLEAGUE_HELP_DESCRIPTION = (
...
def legacy_mcp_tool_name(name: str) -> str:
    raw = str(name or "").strip()
...
def mcp_tool_alias_names(name: str) -> set[str]:
    ...
    aliases = {canonical}
    if is_onecolleague_mcp_tool_name(canonical):
        aliases.add(legacy_mcp_tool_name(canonical))
...
MCP_TOOLS = [
    {
        "name": "onecolleague_help",
        "description": _ONECOLLEAGUE_HELP_DESCRIPTION,
    ...
    {
        "name": "onecolleague_inbox_mark_read",
    ...
    {
        "name": "onecolleague_tracked_send",
        "description": (
            ...
            "For ordinary discussion, quick handoffs, or solo work, use onecolleague_message_send/reply instead."
        ),
        "inputSchema": _obj(
            {
                **_COMMON_GROUP,
    ...
    {
        "name": "onecolleague_message_reply",
    ...
    {
        "name": "onecolleague_voice_secretary_request",
        "description": (
    ...
    {
        "name": "onecolleague_voice_secretary_composer",
        "description": (
    ...
    {
        "name": "onecolleague_code_exec",
        "description": (
            "Preferred/default ChatGPT Web Model tool for non-trivial local development work. "
            "Run JavaScript that orchestrates OneColleague MCP tools through "
            ...
            "If the result says running with a cell_id, call onecolleague_code_wait. "
        ...
        "inputSchema": _obj(
            {
                ...
                "max_output_tokens": {
                    ...
                    "minimum": 1,
                    "maximum": 50000,
                    "description": "Approximate token budget for direct onecolleague_code_exec output.",
                },
            },
        ),
    },
    {
        "name": "onecolleague_code_wait",
        "description": (
            "Wait for or terminate a yielded onecolleague_code_exec cell. Use only when onecolleague_code_exec returned "
            "status=running and a cell_id. Returns new output since the previous yield or the final result."
        ),
        "annotations": {"readOnlyHint": False, "destructiveHint": True},
        "inputSchema": _obj(
            {
                "cell_id": {"type": "string", "description": "Identifier returned by onecolleague_code_exec."},
                "yield_time_ms": {
                    "type": "integer",
                    "default": 10000,
                    "minimum": 0,
    ...
    },
    {
        ...
        "description": (
            ...
            "Use onecolleague_repo_edit or onecolleague_apply_patch for writes."
        ),
        "annotations": {"readOnlyHint": True},
        "inputSchema": _obj(
            {
    ...
    },
    {
        ...
        "description": (
            ...
            "For exact small edits, prefer onecolleague_repo(action=read) -> replace/multi_replace with expected_sha256, then onecolleague_git diff. "
            "Use onecolleague_apply_patch for structural or multi-file Codex *** Begin Patch patches."
        ),
        "annotations": {"readOnlyHint": False, "destructiveHint": True},
        "inputSchema": _obj(
            {
                **_COMMON_GROUP,
                ...
                "expected_sha256": {"type": "string", "description": "Optional sha256 from onecolleague_repo(action=read); rejects stale writes/replaces."},
    ...
    },
    {
        ...
        "description": (
            ...
            "File paths must be relative to the active workspace root. For exact small edits, prefer onecolleague_repo_edit replace/multi_replace with expected_sha256."
        ),
        "annotations": {"readOnlyHint": False, "destructiveHint": True},
        "inputSchema": _obj(
            {
    ...
    },
    {
        ...
        "description": (
            ...
            "Returns ok, returncode, stdout, stderr, and truncation flags. For long-running, streaming, or interactive commands, use onecolleague_exec_command plus onecolleague_write_stdin."
        ),
        "annotations": {"readOnlyHint": False, "destructiveHint": True},
        "inputSchema": _obj(
            {
    ...
    },
    {
        ...
        "description": (
            ...
            "Starts a command and returns output plus session_id when it is still running; use onecolleague_write_stdin to poll or send input. Use onecolleague_shell for short one-shot commands."
        ),
        "annotations": {"readOnlyHint": False, "destructiveHint": True},
        "inputSchema": _obj(
            {
    ...
    },
    {
        ...
        "description": (
            "Writes characters to an existing onecolleague_exec_command session or polls it for more output."
        ),
        "annotations": {"readOnlyHint": False, "destructiveHint": True},
        "inputSchema": _obj(
            {
    ...
    },
    {
        ...
        "description": (
            ...
            "Use onecolleague_shell for unusual git commands."
        ),
        "annotations": {"readOnlyHint": False, "destructiveHint": True},
        "inputSchema": _obj(
            {
    ...
    {
        "name": "onecolleague_presentation",
        "description": (
    ...
    {
        "name": "onecolleague_runtime_list",
        "description": "List available runtimes and runtime pool configuration.",
        "annotations": {"readOnlyHint": True},
        "inputSchema": _obj({}),
    },
    {
        "name": "onecolleague_runtime_wait_next_turn",
        "description": (
            "Pull one coalesced unread OneColleague turn for a website-model runtime actor when no turn was "
            "browser-delivered into the web chat. This does not mark messages read; call "
            "onecolleague_runtime_complete_turn after processing."
        ),
        "annotations": {"readOnlyHint": True},
        "inputSchema": _obj(
            {
    ...
    {
        "name": "onecolleague_runtime_complete_turn",
        "description": (
    ...
    {
        "name": "onecolleague_capability_search",
    ...
    {
        "name": "onecolleague_capability_block",
    ...
    {
        "name": "onecolleague_capability_state",
        "description": "Get caller-effective capability state and visible/dynamic tools.",
        "inputSchema": _obj({**_COMMON_GROUP, **_COMMON_ACTOR}),
    },
    {
        "name": "onecolleague_capability_install",
        "description": (
            "Install a target through the OneColleague capability lifecycle: resolve the target to capability record(s), "
            "import into the registry, enable for the selected actor by default, and return use-ready capability ids. "
            "This is the preferred /install path; future use should go through onecolleague_capability_use/capability_state."
        ),
        "inputSchema": _obj(
            {
                **_COMMON_GROUP,
    ...
    {
        "name": "onecolleague_capability_import",
        "description": (
    ...
    {
        "name": "onecolleague_capability_uninstall",
        "description": (
    ...
    {
        "name": "onecolleague_context_sync",
    ...
    {
        "name": "onecolleague_coordination",
    ...
    {
        "name": "onecolleague_role_notes",
        "description": (
            "Manage actor-scoped role notes in ONECOLLEAGUE_HELP.md (`## @actor: <actor_id>` blocks): action=get|set|clear. "
...
def _alias_text(value: str) -> str:
    ...
    text = text.replace("onecolleague_", "onecolleague_")
...
def _alias_tool_spec(spec: dict) -> dict:
    ...
    item["name"] = legacy_mcp_tool_name(name)
...
LEGACY_MCP_TOOLS = [
    ...
    if isinstance(spec, dict) and is_onecolleague_mcp_tool_name(str(spec.get("name") or ""))
]

MCP_TOOLS_WITH_ALIASES = CANONICAL_MCP_TOOLS + LEGACY_MCP_TOOLS
