...
def _print_web_banner(host: str, port: int) -> None:
    ...
    print(f"[onecolleague]   Local:   http://{_http_host_literal(host)}:{int(port)}", file=sys.stderr)
    lan_ip = _get_lan_ip()
    if lan_ip and lan_ip != host and lan_ip != "127.0.0.1":
        print(f"[onecolleague]   Network: http://{lan_ip}:{int(port)}", file=sys.stderr)
...
def _run_supervised_web(*, host: str, port: int, mode: str, reload: bool, log_level: str, launch_source: str) -> int:
    ...
    if not _check_daemon_running():
        print("error: daemon is not running. Start it with: onecolleague daemon start", file=sys.stderr)
        print("  or use: onecolleague (to start both daemon and web together)", file=sys.stderr)
        return 1
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="onecolleague web", description="OneColleague web port (FastAPI)")
