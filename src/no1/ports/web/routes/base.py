...
from ...mcp.handlers.onecolleague_capability import capability_install as mcp_capability_install
from ...mcp.handlers.onecolleague_capability import capability_use as mcp_capability_use
from ..schemas import (
...
def create_routers(ctx: RouteContext) -> list[APIRouter]:
            ...
            if isinstance(result, dict):
                ...
                if text:
                    ...
                    if isinstance(parsed, dict):
                        ...
                        if canonical_tool_name == "onecolleague_runtime_wait_next_turn":
                        ...
                        elif canonical_tool_name == "onecolleague_runtime_complete_turn":
                            wait_status = str(parsed.get("status") or "").strip()
        ...
        async def _handle_one(item: Any) -> Dict[str, Any]:
            ...
            async def _record_browser_progress(reason: str, detail: str) -> None:
                if not isinstance(connector, dict):
                    return
                if method != "tools/call" or not requested_tool_name or canonical_mcp_tool_name(requested_tool_name) == "onecolleague_runtime_complete_turn":
                    return
                try:
    ...
    async def web_model_mcp_sse_probe(connector_id: str, request: Request) -> StreamingResponse:
        ...
        async def _events():
            yield b": onecolleague web-model connector ready\n\n"
    ...
    async def debug_tail_logs(component: str, group_id: str = "", lines: int = 200) -> Dict[str, Any]:
        """Tail local OneColleague logs (developer mode only)."""
        return await ctx.daemon(
            {
                "op": "debug_tail_logs",
                "args": {
    ...
    async def debug_clear_logs(req: DebugClearLogsRequest) -> Dict[str, Any]:
        """Clear (truncate) local OneColleague logs (developer mode only)."""
        return await ctx.daemon(
            {
                "op": "debug_clear_logs",
                "args": {
    ...
    async def capability_install(group_id: str, request: Request) -> Dict[str, Any]:
        """Install a target through the OneColleague capability lifecycle."""
        if ctx.read_only:
            raise HTTPException(
                status_code=403,
                detail={
