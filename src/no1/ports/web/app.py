...
def create_app() -> FastAPI:
    ...
    async def _lifespan(_app: FastAPI):
        ...
        if restart_supported and runtime_supervisor_pid > 0:
            ...
            supervisor_watchdog_thread = threading.Thread(
                ...
                name="onecolleague-web-supervisor-watchdog",
                daemon=True,
            )
    ...
    if dist:
        ...
        # Prefer packaged UI under `no1/ports/web/dist`.
        try:
    ...
    async def _handle_unexpected_exception(_request: Request, exc: Exception) -> JSONResponse:
        logger.exception("unhandled exception in onecolleague web")
    ...
    from .routes.account import create_routers as create_account_routers
    from .routes.done_hub import create_routers as create_done_hub_routers
