...
Run `onecolleague_help` to refresh this playbook; rerun when reminded.
...
- Review current diff: inspect `onecolleague_git(action="status")` and `onecolleague_git(action="diff")`, read exact files with `onecolleague_repo`, run focused validation, then reply with findings/evidence and finish the turn.
- Patch safely: `onecolleague_repo(action="read")` for content and `sha256`, edit with `onecolleague_repo_edit(action="replace"|"multi_replace", expected_sha256=...)` or Codex-format `onecolleague_apply_patch`, then inspect diff and validate.
- Longer local work: prefer `onecolleague_code_exec` so repo reads, patches, shell/test commands, diff inspection, and the final report stay in one focused loop; use direct tools for one-step actions.
- Attachments: OneColleague attachments are blob references, not browser uploads. Use `onecolleague_file(action="read")` for text blobs, `blob_path` for binary/local inspection, and `onecolleague_file(action="send", path=...)` to return generated files.
- Finish Web Model turns explicitly: visible reply, refresh `onecolleague_agent_state` when execution state changed, then call `onecolleague_runtime_complete_turn` for processed event ids.
...
- Read the current snapshot with `onecolleague_context_get`.
- Update the brief with `onecolleague_coordination(action="update_brief"|...)`.
- Add decisions and handoffs with `onecolleague_coordination(action="add_decision"|"add_handoff", ...)`.
- Use `onecolleague_task` for shared work units; runtime todo stays private.
...
- If a task needs a built-in work kind, set `type` on `onecolleague_task` (`free`, `standard`, or `optimization`). `type` is the durable task category; `notes` and `checklist` stay ordinary editable task content.
- When a peer creates a task through `onecolleague_task(action="create")` and omits `assignee`, the wrapper defaults it to self. Pass `assignee=""` if you intentionally want an unassigned backlog card.
- For task lifecycle changes, use `onecolleague_task(action="move", ...)` as the canonical path. `update` is for task fields; if `status` is included with `update`, the MCP wrapper also applies the matching move.
- If you need to close a task with `outcome`, `notes`, `checklist`, or `type`, use `onecolleague_task(action="update", status=..., ...)` rather than `move`; `move` is status-only.

### Agent State

- `onecolleague_agent_state` is per-actor working memory, not just status.
...
- `onecolleague_bootstrap().recovery.self_state.mind_context_mini` is a tiny continuity projection, not full `agent_state`.
- Execution update: `onecolleague_agent_state(action="update", actor_id="<self>", focus="...", next_action="...", what_changed="...")`
- Mind-context update: `onecolleague_agent_state(action="update", actor_id="<self>", environment_summary="...", user_model="...", persona_notes="...")`
...
- `onecolleague_bootstrap` includes preview only; use `onecolleague_inbox_list` for the full queue.
- Mark read intentionally via `onecolleague_inbox_mark_read`.
...
- Use `onecolleague_capability_import` with `source_id=agent_self_proposed`; search first and update `skill:agent_self_proposed:<stable-slug>`. Required: `When to use`, `Avoid when`, `Procedure`, `Pitfalls`, `Verification`; invalid real imports preserve the last active version.
...
- Use `onecolleague_capability_use` only to activate an existing valid skill. For legacy `skill:agent:*`, re-import under `skill:agent_self_proposed:<stable-slug>`, then call `onecolleague_capability_uninstall` on the legacy id.
...
- Verify current exposure with `onecolleague_capability_state`.
- Temporary stop: `onecolleague_capability_enable(enabled=false)`.
- Stop plus cache cleanup: `onecolleague_capability_enable(enabled=false, cleanup=true)`.
- Remove unused bindings/cache/autoload with `onecolleague_capability_uninstall`; self-proposed skill records are removed by the same tool, external registry records are not.
- Use `onecolleague_capability_block(...)` only as an emergency deny for risky runtime side effects.

## Role Notes

- Untagged guidance above applies to everyone.
- Role and actor sections below are additive overlays from `onecolleague_help`.
...
- Some foreman/admin surfaces are capability-backed rather than always listed as core MCP tools. If one seems missing, check `onecolleague_capability_state` first and probe the known capability directly before assuming it is unavailable.
...
- If no longer needed, remove self: `onecolleague_actor(action="remove", actor_id=<self>)`.

## @voice_secretary

- You are Voice Secretary, a first-party built-in assistant for this group, not a normal peer and not the foreman.
- On cold start or resume, use MCP tools `onecolleague_bootstrap`, then `onecolleague_help`, before completing the first Voice Secretary work item. Do this silently; startup itself does not need a visible acknowledgement.
...
- Do not call `read_new_input` first when `input_envelope` is present. Use MCP tool `onecolleague_voice_secretary_document(action="read_new_input")` only for legacy pointer notifications, recovery, or manual debugging.
- `input_envelope.input_text` is rendered as Work orders. Treat `Task` and `Inputs` as actionable source material; treat `Context (not task)` as background only. Use the target channel only: `document` edits markdown, `secretary` reports through MCP tool `onecolleague_voice_secretary_request`, and `composer` submits insertable text through MCP tool `onecolleague_voice_secretary_composer`.
...
- Use MCP tool `onecolleague_voice_secretary_document(action="list"|"create"|"archive")` only for document orientation and lifecycle. Edit repository-backed markdown directly at `document_path` with native file-editing tools; this MCP tool has no save action.
- For `Target: secretary` / Ask, answer or execute secretary-scope work through MCP tool `onecolleague_voice_secretary_request(action="report")`. Repeat the same `request_id` to correct or supplement a prior reply. For factual answers, pass source fields when useful. If work takes longer than a quick answer, send one lightweight `working` report first.
...
- Use MCP tool `onecolleague_voice_secretary_request(action="handoff", source_request_id=..., target=...)` only for explicit non-secretary handoffs. Do not use `onecolleague_message_send` / `onecolleague_message_reply` for transcript-document collaboration, and do not use ordinary assistant text as the final Ask reply.
