...
async function loadWeixinBundledInternals(packageEntryUrl) {
  ...
  try {
    ...
    const tempPath = path.join(
      os.tmpdir(),
      `onecolleague-weixin-sdk-internals-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mjs`,
    );
    const extra = `
export {
...
export function __onecolleagueGetContextToken(accountId, userId) {
...
async function main() {
  ...
  const markdownToPlainText = typeof runtimeSdk.markdownToPlainText === "function"
    ...
    : (text) => String(text || "");
  const getContextToken = typeof runtimeSdk.__onecolleagueGetContextToken === "function"
    ? runtimeSdk.__onecolleagueGetContextToken
    : null;
    ...
    async chat(request) {
      ...
      const response = await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          pendingReplies.delete(requestId);
          reject(new Error("onecolleague weixin reply timeout"));
        }, 5 * 60 * 1000);
