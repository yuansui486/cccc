...
def _default_entry_lock_path(home: Path) -> Path:
    return home / "daemon" / "onecolleague-app.lock"
...
def _acquire_default_entry_lock(home: Path) -> tuple[Optional[Any], Optional[str]]:
    try:
        ...
        return None, "OneColleague is already running for this home. Stop the existing `onecolleague` session with Ctrl+C, then start it again."
...
def _cleanup_daemon_state_files(home: Path) -> None:
    ...
    for path in (
        daemon_dir / "onecolleagued.sock",
        daemon_dir / "onecolleagued.addr.json",
        daemon_dir / "onecolleagued.pid",
    ):
...
def _same_home_daemon_lock_holder_pids(home: Path) -> list[int]:
    ...
    if not shutil.which("lsof"):
        return []
    lock_path = home / "daemon" / "onecolleagued.lock"
    try:
        proc = subprocess.run(
...
def _same_home_daemon_pids(home: Path) -> list[int]:
    ...
    for proc_dir in proc_dirs:
        ...
        try:
            ...
            continue
        if not ({"no1.daemon_main", "no1.daemon_main"} & set(cmd)) or "run" not in cmd:
            continue
...
def _stop_existing_daemon(home: Path) -> bool:
    ...
    if resp.get("ok"):
        ...
        pid_path = home / "daemon" / "onecolleagued.pid"
        try:
...
def _ensure_daemon_running() -> bool:
    ...
    if resp.get("ok"):
        ...
        if needs_restart:
            ...
            try:
                home = ensure_home()
                sock_path = home / "daemon" / "onecolleagued.sock"
                addr_path = home / "daemon" / "onecolleagued.addr.json"
                pid_path = home / "daemon" / "onecolleagued.pid"
...
def _show_welcome() -> None:
    ...
    print()
    print("Documentation: https://github.com/ChesterRa/onecolleague")
    print("=" * 60)
    print()
...
def _default_entry(*, web_host_override: str = "", web_port_override: Optional[int] = None) -> int:
    ...
    if app_lock is None:
        print(f"[onecolleague] Error: {app_lock_error or 'Could not acquire OneColleague app lock'}", file=sys.stderr)
        return 1

    daemon_process = None
    shutdown_requested = False
    log_path = home / "daemon" / "onecolleagued.log"
    ...
    def _start_daemon() -> bool:
        ...
        for attempt in range(max_attempts):
            if not _stop_existing_daemon(home):
                print("[onecolleague] Failed to stop existing daemon before restart", file=sys.stderr)
                return False
            ...
            try:
                ...
                try:
                    ...
                    pass
                print(f"[onecolleague] Failed to start daemon: {e}", file=sys.stderr)
                return False
            ...
            for _ in range(50):
                ...
                if ret is not None:
                    ...
                    if attempt + 1 < max_attempts:
                        if still_running and int(ret) == 0:
                            print("[onecolleague] Another daemon remained active during startup; retrying clean restart...", file=sys.stderr)
                        else:
                            print("[onecolleague] Daemon exited during startup; retrying clean restart...", file=sys.stderr)
                        break
                    if still_running and int(ret) == 0:
                        print("[onecolleague] Another daemon is still active after clean restart attempts.", file=sys.stderr)
                        return False
                    print(f"[onecolleague] Daemon exited during startup (exit code {ret}). Check log: {log_path}", file=sys.stderr)
                    try:
                ...
                if attempt + 1 < max_attempts:
                    print("[onecolleague] Daemon did not become ready in time; retrying clean restart...", file=sys.stderr)
                    continue
                print("[onecolleague] Daemon failed to become ready in time", file=sys.stderr)
                return False
    ...
    _lifecycle = DaemonLifecycle(
        ...
        log=lambda msg: print(f"[onecolleague] {msg}", file=sys.stderr),
    ...
    # Saved binding is the baseline, but top-level `onecolleague --host/--port`
    ...
    try:
        if not _stop_existing_web_runtime(home):
            print("[onecolleague] Failed to stop existing web runtime before restart", file=sys.stderr)
            return 1

        print("[onecolleague] Starting daemon...", file=sys.stderr)
        if not _start_daemon():
            print("[onecolleague] Error: Could not start daemon", file=sys.stderr)
            return 1
        # Sync initial process reference to lifecycle helper.
        _lifecycle.process = daemon_process
        print("[onecolleague] Daemon started", file=sys.stderr)
        ...
        def _print_web_banner(cur_host: str, cur_port: int) -> None:
            print("[onecolleague] Starting web server...", file=sys.stderr)
            print(f"[onecolleague]   Local:   http://{_http_host_literal(cur_host)}:{cur_port}", file=sys.stderr)
            lan_ip = _get_lan_ip()
            if lan_ip and lan_ip != cur_host and lan_ip != "127.0.0.1":
                print(f"[onecolleague]   Network: http://{lan_ip}:{cur_port}", file=sys.stderr)
        ...
        if web_process is None:
            print(f"[onecolleague] Error: {web_error or 'Could not start web server'}", file=sys.stderr)
            return 1
        ...
        while True:
            ...
            if int(ret or 0) == WEB_RUNTIME_RESTART_EXIT_CODE:
                print("[onecolleague] Applying saved Web binding changes...", file=sys.stderr)
