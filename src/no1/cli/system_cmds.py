...
def _find_installed_distribution() -> Any:
    """Return the installed OneColleague distribution."""
    for dist_name in (_UPDATE_PACKAGE_NAME,):
        try:
...
def _blocked_update_message(install_kind: str) -> str:
    ...
    if install_kind == "editable":
        return "editable installs are not updated automatically by `onecolleague update`"
    if install_kind == "local_path":
        return "local path installs are not updated automatically by `onecolleague update`"
    return "this install source is not updated automatically by `onecolleague update`"
...
def cmd_update(args: argparse.Namespace) -> int:
    """Upgrade the current OneColleague installation in-place via the active Python runtime."""
    try:
        ...
        _print_json(
            {
                ...
                "error": {
                    ...
                    "message": "OneColleague is not installed in the current Python environment",
                },
            }
        )
        return 1
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    # OneColleague home
    home = ensure_home()
    print(f"ONECOLLEAGUE_HOME: {home}")
    ...
    if available_count == 0:
        ...
        print("First-class supported runtimes: claude, codex, droid, amp, auggie, neovate, gemini, hermes, kimi")
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    onecolleague_cmd = get_onecolleague_mcp_stdio_command()
    ...
    def _manual_setup(rt: str, *, runtime_available: bool) -> None:
        cmd = build_mcp_add_command(rt) or onecolleague_cmd
        display_cmd = cmd
    ...
    for rt in runtimes_to_setup:
        if rt in auto_mcp_runtimes:
            ...
            results["mcp"]["custom"] = {
                "mode": "manual",
                "hint": f"Add an MCP stdio server named 'onecolleague' that runs: {_cmd_line(onecolleague_cmd)}",
            }
...
def cmd_daemon(args: argparse.Namespace) -> int:
    if args.action == "status":
        ...
        if resp.get("ok"):
            r = resp.get("result") if isinstance(resp.get("result"), dict) else {}
            print(f"onecolleagued: running pid={r.get('pid')} version={r.get('version')}")
            return 0
        print("onecolleagued: not running")
        return 1

    if args.action == "start":
        if _ensure_daemon_running():
            print("onecolleagued: running")
            return 0
        print("onecolleagued: failed to start")
        return 1

    if args.action == "stop":
        resp = call_daemon({"op": "shutdown"})
        if resp.get("ok"):
            print("onecolleagued: shutdown requested")
            return 0
        print("onecolleagued: not running")
        return 0
