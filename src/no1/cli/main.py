...
def build_parser() -> argparse.ArgumentParser:
    prog = os.path.basename(sys.argv[0] or "") or "onecolleague"
    ...
    p_update = sub.add_parser("update", help="Update OneColleague in the current Python environment")
    ...
    p_runtime_hermes_prepare = hermes_sub.add_parser("prepare", help="Configure OneColleague MCP in the selected Hermes profile")
    p_runtime_hermes_prepare.add_argument("--path", default=".", help="Project path for Hermes MCP discovery (default: current directory)")
    p_runtime_hermes_prepare.add_argument("--yes", action="store_true", help="Enable all discovered OneColleague MCP tools during Hermes setup")
    ...
    p_runtime_hermes_mcp_test = hermes_sub.add_parser("mcp-test", help="Run Hermes MCP test with OneColleague actor env")
