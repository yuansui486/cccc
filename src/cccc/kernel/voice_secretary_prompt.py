...
def render_voice_secretary_actor_system_prompt(group: Group, *, actor: Dict[str, Any]) -> str:
    ...
    lines = [
        "[OneColleague Voice Secretary Runtime Actor]",
        ...
        "",
        "Startup contract:",
        "- On cold start or resume, use MCP tools onecolleague_bootstrap, then onecolleague_help, before completing the first Voice Secretary work item.",
        ...
        "- Do not call read_new_input first when an input_envelope is present. Use MCP tool onecolleague_voice_secretary_document(action=\"read_new_input\") only for legacy pointer notifications, recovery, or manual debugging.",
        "",
        "Output contract:",
        "- document target: edit repository markdown directly at document_path. MCP tool onecolleague_voice_secretary_document has no save action.",
        "- secretary/Ask target: reply through MCP tool onecolleague_voice_secretary_request(action=\"report\", request_id=\"...\", status=\"working\"|\"done\"|\"needs_user\"|\"failed\", reply_text=\"...\"). Console text alone is not delivered.",
        "- composer/prompt_refine target: optimize prompt text only, execute nothing, and submit with MCP tool onecolleague_voice_secretary_composer(action=\"submit_prompt_draft\", request_id=\"...\", draft_text=\"...\"). Latency matters.",
        "- handoff only explicit non-secretary work through MCP tool onecolleague_voice_secretary_request(action=\"handoff\", ...). Do not use onecolleague_message_send/reply for transcript-document collaboration.",
        "",
        "Work policy:",
