...
interface SteeringSummaryCardProps {
  tr: ContextTranslator;
  ui: ContextModalUi;
  brief: CoordinationBrief | null;
  editingBrief: boolean;
...
interface SteeringActivityCardProps {
  tr: ContextTranslator;
  ui: ContextModalUi;
  activityBusyKind: "decision" | "handoff" | null;
  activityError: string;
...
export function SteeringSummaryCard({
  tr,
  ui,
  brief,
  editingBrief,
...
export function SteeringActivityCard({
  tr,
  ui,
  activityBusyKind,
  activityError,
  ...
  return (
    <section className={classNames("rounded-xl border p-4", "glass-card")}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className={classNames("text-sm font-semibold", "text-[var(--color-text-primary)]")}>{tr("context.activityLog", "Activity")}</div>
      ...
      {activityError ? <div className={classNames("mt-3 rounded-xl border px-3 py-2 text-sm", "border-rose-500/30 bg-rose-500/15 text-rose-600 dark:text-rose-400")}>{activityError}</div> : null}
        ...
        <div className={classNames("flex flex-col gap-3 border-t pt-4", "border-[var(--glass-border-subtle)]")}>
          ...
          {steeringTab === "summary" ? (
            <SteeringSummaryCard
              ...
              briefDraft={briefDraft}
              syncBusy={syncBusy}
              onStartBriefEdit={onStartBriefEdit}
              onCancelBriefEdit={onCancelBriefEdit}
          ...
          {steeringTab === "log" ? (
            <SteeringActivityCard
              ...
              onAddCoordinationNote={onAddCoordinationNote}
            />
          ) : null}
        </div>
      </div>
