...
class TestInboxReadOps(unittest.TestCase):
    ...
    def test_unsupported_internal_actor_does_not_match_peer_or_broadcast_chat_targets(self) -> None:
        ...
        try:
            ...
            group_id = create_group(registry, title="legacy-internal-routing", topic="").group_id
            ...
            direct_event = {"kind": "chat.message", "by": "lead", "data": {"to": ["legacy-internal"], "text": "direct ping"}}

            self.assertTrue(is_message_for_actor(group, actor_id="peer1", event=peers_event))
            self.assertFalse(is_message_for_actor(group, actor_id="legacy-internal", event=peers_event))
            self.assertFalse(is_message_for_actor(group, actor_id="legacy-internal", event=all_event))
            self.assertFalse(is_message_for_actor(group, actor_id="legacy-internal", event=broadcast_event))
            self.assertTrue(is_message_for_actor(group, actor_id="legacy-internal", event=direct_event))
        finally:
            cleanup()
