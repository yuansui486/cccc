...
class TestCliMain(unittest.TestCase):
    ...
    def test_main_applies_top_level_port_override_to_default_entry_only_for_invocation(self) -> None:
        ...
        old_port = os.environ.get("ONECOLLEAGUE_WEB_PORT")

        def _assert_port_and_return(*, web_host_override: str, web_port_override: int | None) -> int:
            self.assertEqual(os.environ.get("ONECOLLEAGUE_WEB_PORT"), "9000")
        ...
        try:
            ...
            if old_port is None:
                os.environ.pop("ONECOLLEAGUE_WEB_PORT", None)
            else:
                os.environ["ONECOLLEAGUE_WEB_PORT"] = old_port

        self.assertEqual(rc, 0)
        mock_default.assert_called_once_with(web_host_override="", web_port_override=9000)
        self.assertEqual(os.environ.get("ONECOLLEAGUE_WEB_PORT"), old_port)
