...
class TestSystemCmdsSetup(unittest.TestCase):
    def test_cmd_setup_codex_reports_added_via_shared_install_helper(self) -> None:
        ...
        with patch("no1.kernel.runtime.get_onecolleague_mcp_stdio_command", return_value=[r"C:\CCCC\no1.exe", "mcp"]), patch(
            "no1.kernel.runtime.detect_runtime",
            return_value=SimpleNamespace(available=True, path=r"C:\Tools\codex.cmd"),
        ), patch("no1.daemon.mcp_install.is_mcp_installed", return_value=False), patch(
    ...
    def test_cmd_setup_kimi_reports_present_when_already_configured(self) -> None:
        ...
        ), patch("no1.daemon.mcp_install.is_mcp_installed", return_value=True), patch(
            "no1.daemon.mcp_install.ensure_mcp_installed",
            return_value=True,
        ), patch.object(
            system_cmds,
    ...
    def test_cmd_setup_claude_manual_hint_uses_resolved_executable_path(self) -> None:
        ...
        resolved_cmd = [r"C:\Users\tester\.local\bin\claude.exe", "mcp", "add", "-s", "user", "onecolleague", "--", r"C:\CCCC\no1.exe", "mcp"]

        with patch("no1.kernel.runtime.get_onecolleague_mcp_stdio_command", return_value=[r"C:\CCCC\no1.exe", "mcp"]), patch(
            "no1.daemon.mcp_install.build_mcp_add_command",
            return_value=["claude", "mcp", "add", "-s", "user", "onecolleague", "--", r"C:\CCCC\no1.exe", "mcp"],
        ), patch(
            "no1.kernel.runtime.detect_runtime",
