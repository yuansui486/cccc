...
class TestPromptDefaults(unittest.TestCase):
    ...
    def test_builtin_help_is_compact(self) -> None:
        ...
        self.assertIn('capability_id="pack:context-advanced"', body)
        self.assertIn('capability_id="pack:computer-control-local"', body)
        self.assertIn("group/actor -> `pack:group-runtime`", body)
        self.assertIn("capability administration -> `pack:capability-admin`", body)
        self.assertIn('tool_name="onecolleague_computer_control_catalog"', body)
        self.assertIn('tool_name="onecolleague_computer_recording"', body)
        self.assertIn('tool_name="onecolleague_computer_workflow"', body)
        self.assertIn('tool_name="onecolleague_computer_run"', body)
        self.assertIn("Copy the current `turn_grant_receipt` object into the target tool's inner `tool_arguments`", body)
